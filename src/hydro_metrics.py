"""Hydrological metrics calculation module.

This module provides comprehensive hydrological metrics and base flow index (BFI)
calculations for discharge time series analysis in the CAMELS-RU dataset.

Note:
    Calculations are performed on continuous series without gaps.
    If gaps exist in the overall series, it is split into n-series
    depending on the gaps. BFI calculations are performed for
    individual years of observations.
"""
...
logger = setup_logger("hydro_metrics", log_file="logs/hydrological.log")
###################################################################
...
def hydro_job(hydro_year: pd.Series, calendar_year: pd.Series):
    ...
    hfd_date = hfd_calc(calendar_year, hydro_year)
    hydro_quantile = q5_q95(hydro_year)
    # high_flow_freq = high_q_freq(hydro_year)

    # plot results
    hydro_year.plot(label="discharge, cms", ylim=0)
    plt.axhline(y=hydro_mean, c="r", label="average hydro")

    plt.axhline(y=hydro_quantile["q5"], c="green", ls="-.", label="5% quantile")

    plt.axhline(y=hydro_quantile["q95"], c="lime", ls="-.", label="95% quantile")

    plt.plot(bfi_dates, c="y", label="baseflow")
    ...
    for i, period in enumerate(high_q_dur(hydro_year)):
        plt.plot(period, ls=":", lw=2, c="black", label="high flow occasions" if i == 0 else "")

    for i, period in enumerate(low_q_dur(hydro_year)):
        plt.plot(period, ls=":", lw=2, c="cyan", label="low flow occasions" if i == 0 else "")
