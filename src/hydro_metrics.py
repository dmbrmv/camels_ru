...
def bfi(Q, alpha, passes, reflect):
    ...
    if len(Q)-1 > reflect:
        Q_reflect = np.array([np.float64(np.NaN) for _ in range(
            len(Q) + 2 * reflect)], dtype=np.float64)

        Q_reflect[:reflect] = Q[(reflect):0:-1]
        Q_reflect[(reflect):(reflect + len(Q))] = Q
        Q_reflect[(reflect + len(Q)):(len(Q) + 2 + 2 * reflect)
                  ] = Q[len(Q)-2:len(Q) - reflect - 2:-1]

    else:
        Q_reflect = np.array([np.float64(np.NaN)
                             for _ in range(len(Q))], dtype=np.float64)
        Q_reflect = Q
...
def bfi_1000(Q, passes, reflect):
    ...
    random.seed(1996)
    alpha_coefficients = [np.float64(
        random.uniform(0.9, 0.98)) for i in range(1000)]
...
def slope_fdc_gauge(hydro_year: pd.Series):
    ...
    slope_fdc = (math.log(np.nanpercentile(hydro_year, q=100-33)) -
                 math.log(np.nanpercentile(hydro_year, q=100-66)))/(0.66-0.33)

    return slope_fdc


def hfd_calc(calendar_year: pd.Series,
...
def low_q_dur(hydro_year: pd.Series):
    ...
    mean_masks = np.ma.clump_unmasked(np.ma.masked_where(hydro_year > mean_lim,
                                                         hydro_year))
...
def hydro_job(hydro_year: pd.Series,
    ...
    bfi_mean, bfi_dates = bfi_1000(hydro_year.to_numpy(),
                                   3, 30)
    bfi_dates = pd.Series(bfi_dates,
                          index=hydro_year.index)
