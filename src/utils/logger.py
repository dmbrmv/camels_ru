...
# --- Constants ---
_DEFAULT_LOGGER_NAME = "camels_ru"
_DEFAULT_LOG_DIR = "logs"
_DEFAULT_LOG_FILENAME = "camels_ru.log"
...
class EmojiFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        record.emoji = _LEVEL_EMOJIS.get(record.levelno, "➡️  ") if self.use_emoji else ""
...
def _determine_log_level(explicit_level: int | str | None) -> int:
    ...
    """
    level_str = str(explicit_level or os.getenv("camels_ru_LOG_LEVEL", "INFO")).upper()
...
def get_logger(name: str = _DEFAULT_LOGGER_NAME, *, level: int | str | None = None) -> logging.Logger:
    ...
    formatter = EmojiFormatter(use_color=use_color, use_emoji=use_emoji, indent_multiline=True)
...
def setup_logger(
    ...
    *,
    ...
    effective_log_file = (
        log_file or os.getenv("camels_ru_LOG_FILE") or Path(_DEFAULT_LOG_DIR) / _DEFAULT_LOG_FILENAME
    )
    ...
    if rotate:
        ...
        handler_exists = any(
            isinstance(h, RotatingFileHandler) and h.baseFilename == abs_log_path
        ...
        if not handler_exists:
            try:
                ...
                rfh.setFormatter(
                    EmojiFormatter(use_color=False, use_emoji=False, indent_multiline=False)
                )
